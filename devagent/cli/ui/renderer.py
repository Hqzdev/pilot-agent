...
from devagent.agent.phases import Phase
from devagent.agent.types import Message, ToolCall, ToolResult
from devagent.cli.ui.components import create_console
from devagent.cli.ui.input import DevAgentInput
from devagent.cli.ui.status import StatusBar
from devagent.cli.ui.theme import Glyphs, glyphs
...
class SpinnerContext(AbstractContextManager[Progress]):
    def __init__(self, console: Console, text: str):
        self.progress = Progress(
            SpinnerColumn(Glyphs.SPINNER, style="devagent.muted"),
            TextColumn(text, style="devagent.muted"),
            console=console,
            transient=True,
        )
...
class Renderer:
    ...
    def render_tool_call(self, call: ToolCall) -> None:
        ...
        self.console.print(Text(f"{g.TOOL} {call.name} · {arg}", style="devagent.muted"))
    ...
    def render_tool_result(
        ...
        *,
        ...
        status_style = "devagent.ok" if ok else "devagent.err"
        name = "skill" if call.name == "load_skill" else call.name
        suffix = self._tool_suffix(call, result, exit_code)
        line = Text(f"{g.TOOL} {name} · ", style="devagent.muted")
        line.append(arg, style="devagent.accent" if call.name == "load_skill" else "devagent.muted")
        line.append(" · ", style="devagent.muted")
        line.append(status_glyph, style=status_style)
        line.append(f" {suffix} · {elapsed_s:.1f}s", style="devagent.muted")
        delta = _line_delta(result.content)
        if delta and call.name in {"write_file", "edit_file"}:
            line.append(f" · {delta}", style="devagent.info")
    ...
    def render_error_tail(self, content: str) -> None:
        for line in _tail_lines(content):
            tail = Text("│ ", style="devagent.muted")
            tail.append(line, style="devagent.err")
    ...
    def render_phase_transition(
        ...
        *,
        ...
        g = glyphs()
        self.console.print(Rule(style="devagent.accent.dim"))
        self.console.print(
            Text.assemble(
                (f"{g.PHASE} Phase · {phase} ", "devagent.accent"),
                (f"{g.ARROW} ", "devagent.muted"),
                (g.OK, "devagent.ok"),
                (" complete", "devagent.muted"),
            )
        )
        if summary:
            self.console.print(Text(_truncate_middle(summary, 100), style="devagent.muted"))
        if next_phase:
            self.console.print(
                Text(
                    f"{g.PHASE} Phase · {next_phase} — starting",
                    style="devagent.accent",
                )
            )
        self.console.print(Rule(style="devagent.accent.dim"))
...
class UI:
    def __init__(
        ...
        *,
        ...
        self.input = DevAgentInput(history_path=history_path, skill_names=skill_names or [])
    ...
    def notice(self, text: str) -> None:
        self.console.print(Text(text, style="devagent.muted"))

    def success(self, text: str) -> None:
        self.console.print(Text(text, style="devagent.ok"))

    def warning(self, text: str) -> None:
        self.console.print(Text(text, style="devagent.warn"))

    def error(self, text: str) -> None:
        self.console.print(Text(text, style="devagent.err"))
