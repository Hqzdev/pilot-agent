    ...
    def resolve_key(self) -> str:
        ...
        if not value:
            raise RuntimeError(
                ...
                f"Run: devagent setup or export {self.api_key_env}=<api-key>"
            )
        return value
...
def project_config_path(project_root: Path | None = None) -> Path:
    return (project_root or Path(".")).resolve() / ".devagent" / "config.yaml"
...
def _env_map() -> dict[str, str]:
    return {
        ...
        "tool_timeout_s": "DEVAGENT_TOOL_TIMEOUT_S",
...
def _model_data(cfg: DevAgentConfig) -> dict[str, Any]:
    data = cfg.model_dump(exclude={"sources"})
    return data


def flatten_config(cfg: DevAgentConfig) -> dict[str, Any]:
...
def load_config(
    *,
    ...
    if model is not None:
        data["model"] = model
        sources["model"] = "cli"
    cfg = DevAgentConfig(**data)
    cfg.sources = sources
    return cfg


def config_value(cfg: DevAgentConfig, key: str) -> Any:
...
def set_config_value(path: Path, key: str, raw_value: str) -> DevAgentConfig:
    ...
    DevAgentConfig(**merged)
    write_yaml_atomic(path, candidate)
    return load_config(config_path=path)


def update_config_values(path: Path, updates: dict[str, Any]) -> DevAgentConfig:
    current = _load_yaml(path)
    candidate = dict(current)
