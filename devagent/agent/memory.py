...
DEPLOY_SYNTHESIS_PROMPT = (
    "Фаза deploy завершена. Просмотри проделанную работу. Есть ли воспроизводимая\n"
    "процедура (последовательность команд/правок), которой НЕТ в загруженных скиллах\n"
    "и которая пригодится в других проектах? Если да — вызови save_skill со скиллом\n"
    "в стандартном формате (frontmatter: source: learned). Если нет — вызови\n"
    "complete_phase."
)

LESSON_PROMPT = """Извлеки урок из цикла исправления ошибки.
Ошибка: {stderr_tail}
Действия: {actions}
Формат ответа строго:
PROBLEM: одна строка
FIX: 1-3 строки, конкретные команды/правки
TAGS: 2-4 тега стека через запятую
Если урок тривиален (опечатка, забытый импорт) или специфичен только
для этого проекта — ответь одним словом: SKIP"""
...
def _default_home() -> Path:
    return Path(os.environ.get("DEVAGENT_HOME", "~/.devagent")).expanduser()
...
def _has_deploy_synthesis_prompt(history: list[Message]) -> bool:
    return any(message.content.startswith("Фаза deploy завершена.") for message in history)
