...
class StatusBar:
    ...
    def render(self, state: StatusState) -> Text:
        ...
        text.append(" · ", "pilot_agent.muted")
        text.append(self._todo_segment(state), "pilot_agent.info")
        text.append(" · ", "pilot_agent.muted")
        ctx_style = "pilot_agent.warn" if state.context_percent >= 80 else "pilot_agent.muted"
        text.append(f"ctx {state.context_percent}%", ctx_style)
        cost = self._cost(state)
        if cost is not None:
            text.append(" · ", "pilot_agent.muted")
            text.append(f"${cost:.2f}", "pilot_agent.muted")
        text.append(" · ", "pilot_agent.muted")
        text.append(state.model, "pilot_agent.muted")
        return text

    def live(self, label: str) -> Live:
        renderable = Text(label, style="pilot_agent.muted") if self.enabled else Text("")
