...
from pilot_agent.backends.local import ensure_command_allowed
...
class DockerProcessHandle(ProcessHandle):
    def __init__(self, backend: DockerBackend, ident: str):
        ...
        self.ident = ident
        self.log_path = f"/tmp/pilot-agent-{ident}.log"
        self.pid_path = f"/tmp/pilot-agent-{ident}.pid"
        self.exit_path = f"/tmp/pilot-agent-{ident}.exit"
...
class DockerBackend(ExecutionBackend):
    ...
    def __init__(
        ...
        *,
        ...
        self.container_name = f"pilot-agent-sbx-{self.session_id}"
        self._started = False
    ...
    def healthcheck(self) -> BackendCheckResult:
        if not _docker_available():
            return BackendCheckResult(
                ...
                "Install/start Docker or run: pilot-agent backend local",
            )
        ...
        return BackendCheckResult("fail", "backend docker", probe.output, "Run: pilot-agent doctor")
